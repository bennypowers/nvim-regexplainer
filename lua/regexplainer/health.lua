...
function M.check()
  ...
  if parser_ok then
    health.ok('regex parser is installed')
  else
    health.error('regex parser is not installed', {
      'Install the regex parser for treesitter',
    })
  end

  -- Check regex injection per language
  for _, sample in ipairs(injection_samples) do
    ...
    if not lang_ok then
      health.warn(filetype .. ': parser not installed (skipped)')
    else
      if test_regex_injection(filetype, source, row, col) then
        health.ok(filetype .. ': regex injection working')
      else
        health.error(filetype .. ': regex injection not detected', {
